...
export function AIChatBox({
  ...
  const liveStatus = livePhase === "listening" && liveListening ? getLiveVoiceStatus("listening") : getLiveVoiceStatus(livePhase);
  ...
  return (
    <section aria-label="Icynigma conversation" className={cn("flex flex-col overflow-hidden rounded-xl border border-purple-500/20 bg-card/80 text-card-foreground shadow-2xl", className)} style={{ height }}>
      ...
      <form onSubmit={handleSubmit} className="border-t border-purple-500/15 bg-slate-950/40 p-3 sm:p-4">
        {(liveActive || livePhase === "error" || livePhase === "unsupported") && <div className={cn("mb-2 flex items-center gap-2 rounded-lg border px-3 py-2 text-xs", livePhase === "error" || livePhase === "unsupported" ? "border-red-400/25 bg-red-500/10 text-red-200" : "border-cyan-300/25 bg-cyan-500/10 text-cyan-50")} role={livePhase === "error" || livePhase === "unsupported" ? "alert" : "status"} aria-live="polite"><Radio className={cn("size-3.5 shrink-0", livePhase === "listening" && "animate-pulse")} aria-hidden="true" /><span>{liveStatus}</span></div>}
